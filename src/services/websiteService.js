...
export const websiteService = {
  ...
  createWebsite: async (domain) => {
    const { data } = await api.post("/api/v1/analytics/create", { domain });
    return data;
  },
  ...
  getTimeseries: async (websiteId) => {
    const { data } = await api.get(`/api/v1/analytics/timeseries/${websiteId}`);
    return data;
  },

  getSessions: async (websiteId) => {
    const { data } = await api.get(`/api/v1/analytics/sessions/${websiteId}`);
    return data;
  },
  ...
  getPageViewsData: async (websiteId) => {
    const { data } = await api.get(`/api/v1/analytics/pageviews/${websiteId}`);
    return data;
  },
};