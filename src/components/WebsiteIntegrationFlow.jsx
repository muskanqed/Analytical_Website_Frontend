...
const WebsiteIntegrationFlow = ({ showDialog, setShowDialog }) => {
  ...
  const trackingScript = `<script>
    window.analyticsConfig = {
      ...
      endpoint: 'http://localhost:8081/api/v1/track',
      respectDoNotTrack: true
    };
  </script>
  <script async src="http://localhost:8081/tracking.js"></script>`;

  const handleContinue = async () => {
    try {
      const data = await AddWebsite(newWebsite);
      if (data) {
        setResponse(data);
        setCurrentStep(1);
      }
    ...
    <Dialog open={showDialog} onOpenChange={setShowDialog}>
      <DialogContent className="max-w-[640px]">
        <DialogHeader className="pb-4">
          ...
          <Progress value={(currentStep + 1) * 33.33} className="h-1.5" />
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          {currentStep === 1 && (
            <p className="text-xs text-muted-foreground text-center px-8">
              Need help? Contact our support team or visit our documentation for
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base flex items-center gap-2">
            <ShieldCheck className="h-4 w-4" /> Step 2: Verify Installation
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <p className="text-sm text-muted-foreground">
            After installing the script, visit your website to trigger initial
            data collection
          </p>
          <div className="flex flex-col gap-2">
            <Button onClick={onComplete}>I've Installed the Script</Button>
            <Button
              variant="outline"
    ...
    <div className="flex flex-col items-center text-center gap-4">
      ...
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground">
          Analytics data will start appearing in your dashboard within 24 hours.
          <br />
